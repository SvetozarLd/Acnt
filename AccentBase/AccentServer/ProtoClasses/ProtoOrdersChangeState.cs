...
namespace AccentServer.ProtoClasses
{
    class ProtoOrdersChangeState
    {
        ...
        public byte[] protoSerialize(protoRowsList items)
        {
            ...
            try
            {
                using (var stream = new MemoryStream()) { Serializer.SerializeWithLengthPrefix<protoRowsList>(stream, items, PrefixStyle.Base128, Serializer.ListItemTag); result = stream.ToArray(); }
                return result;
            }
            catch { return null; }
        }
